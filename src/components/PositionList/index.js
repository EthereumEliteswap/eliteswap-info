...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name eliteswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name eliteswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name eliteswap';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  ELITESWAP_RETURN: 'ELITESWAP_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.ELITESWAP_RETURN) {
          return p0?.eliteswap?.return > p1?.eliteswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.ELITESWAP_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.ELITESWAP_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.ELITESWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
